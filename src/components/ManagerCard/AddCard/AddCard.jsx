...
function AddCard({ setAddWindow, setAnim }) {
    ...
    useEffect(() => {
        const stage1 = !Object.values(workerInfo).slice(1, 9).some(el => el.length < 2)
    ...
    const handleCreateCard = () => {
        ...
        dispatch(setFormat('Офис'))
        dispatch(setStartDate(''))
        dispatch(setShedule(0))
        dispatch(setShedule2(0))
    ...
    return (
        <div className={s.add}>
            ...
            <button onClick={activeStage == 3 ? handleCreateCard : handleNextStage} className={`${s.button} ${(filledStage < activeStage && activeStage !== 3) && s.button_dis}`}>{activeStage == 3 ? 'Создать карточку' : 'Далее'}</button>
        </div>
    )
};
