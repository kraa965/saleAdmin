...
export const handleDateDifferenceLeader = (d) => {
    ...
    if (day == day2 && month == month2 && year == year2) {
        return `Сегодня в ${hour < 10 ? `0${hour}` : hour}:${minute < 10 ? `0${minute}` : minute}`;
    }
    ...
    if ((day - day2 > 1) || (month !== month2) || (year !== year2)) {
        return `${day}.${month < 10 ? `0${month}` : month} ${hour < 10 ? `0${hour}` : hour}:${minute < 10 ? `0${minute}` : minute}`
    }
}
...
export const handleDatePlan = (d) => {
    ...
    if (day - dayNow == 1 && month == monthNow && year == yearNow) {
    ...
    if (day - dayNow < 0 && month == monthNow && year == yearNow) {
        return `на ${day} ${fMonth} в ${time}`
    }

    if (month !== monthNow && year !== yearNow) {
        return `на ${day} ${fMonth} ${year} года`
    }
